...
def main(unused_argv):
  ...
  def render_eval_fn(variables, _, rays):
    return jax.lax.all_gather(
        model.apply(
            ...
            compute_extras=True,
            compute_reg=False,
            cameras=dataset.worldtocams if type(dataset) == datasets.DTU else None), axis_name='batch')
            #cameras=dataset.worldtocams), axis_name='batch')
  ...
  last_step = 0
  out_dir = config.out_dir if config.out_dir != '' else path.join(config.checkpoint_dir,
  ...
  while True:
    ...
    for name in list(metrics[0].keys()):
      print(f'average %s = %f', name, np.asarray([m[name] for m in metrics]).mean())
    ...
    if (config.eval_save_output and (not config.render_path) and
      ...
      for name in list(metrics[0].keys()):
        with utils.open_file(path_fn(f'metric_{name}_{step}.txt'), 'w') as f:
          ...
          f.write('\navg: '+ str(np.asarray([m[name] for m in metrics]).mean()))
