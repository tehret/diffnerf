...
def diff_clip(v, m):
    # Maximum to avoid division by zero in where since both terms are always computed
    vals = jnp.maximum(jax.lax.stop_gradient(v), 1e-6)
    return jnp.where(vals < m, v, m*v/vals)
...
def train_step(
  ...
  def loss_fn(variables):
    ...
    for rendering in renderings:
      ...
      # Loss 
      # No clipping when saturated
      numer = (lossmult[:,0] * jnp.where(jnp.logical_or(batch['rgb'][Ellipsis, :3].sum(axis=-1) < 0.25, batch['rgb'][Ellipsis, :3].sum(axis=-1) == 3.), 
          (rendering['reg']**2).sum(axis=-1), diff_clip((rendering['reg']**2).sum(axis=-1), config.reg_max))).sum()
      denom = lossmult.sum()
      losses_reg.append(numer / denom)

    render_random_rays = ((config.depth_tvnorm_loss_mult != 0.0) or
                          (config.depth_tvnorm_decay))

    if render_random_rays:
      lossmult_rand = batch['rays_random'].lossmult
      ...
      for rendering in renderings_random:
          numer = (lossmult_rand * diff_clip((rendering['reg']**2).sum(axis=-1), config.reg_max)).sum()
          denom = lossmult_rand.sum()
          losses_georeg.append(numer / denom)
    else:
        losses_georeg.append(0.0)

    losses = jnp.array(losses)
    losses_georeg = jnp.array(losses_georeg)
    losses_reg = jnp.array(losses_reg)
    ...
    loss = (
        ...
        config.reg_weight * (losses_reg[-1] + config.coarse_loss_mult * jnp.sum(losses_reg[:-1])))
    return loss, (losses, disp_mses, normal_maes, weight_l2, losses_georeg, losses_reg)

  (loss, loss_aux), grad = (jax.value_and_grad(loss_fn, has_aux=True)(
      state.optimizer.target))
  (losses, disp_mses, normal_maes, weight_l2, losses_georeg, losses_reg) = loss_aux
  ...
  losses_reg = jax.lax.pmean(losses_reg, axis_name='batch')
  ...
  for step, batch in zip(range(init_step, config.max_steps + 1), pdataset):
    ...
    if jax.host_id() == 0:
      ...
      if step % config.print_every == 0:
        ...
        for i, l in enumerate(stats.losses_reg):
          summary_writer.scalar(f'train_losses_reg{i}', l, step)
