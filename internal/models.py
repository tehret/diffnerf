...
class MipNerfModel(nn.Module):
  ...
  num_cams: int = 1  # Set density to 0 of samples that are not seen by at least this number of camera (only used for novel view synthesis)
  ...
  def __call__(
    ...
    def apply_fn(key, i_level, t_vals, weights, origins, directions, radii, near, far, viewdirs):
      if i_level == 0:
      ...
      if self.use_viewdirs:
        (rgb, density, normals) = mlp(rng, samples, viewdirs)
      else:
        (rgb, density, normals) = mlp(rng, samples, None)

      if cameras is not None:
          world = jnp.concatenate((samples[0], jnp.ones((samples[0].shape[0], 1))), axis=-1)
          count = jnp.full_like(density, 0.)
          for cam in cameras:
              idxs = cam @ world.T
              idxs = idxs / idxs[2,:]
              count = count + jnp.logical_and(jnp.logical_and(idxs[0,:] >= 0., idxs[1,:] >= 0.), jnp.logical_and(idxs[0,:] < 400., idxs[1,:] < 300.))
          density = jnp.where(count >= self.num_cams, density, 0.)

      # Volumetric rendering.
      weights, _, _, delta = mip.compute_alpha_weights(
          density, t_vals, directions)
      ...
      return rendering['distance_mean'], (t_vals, weights, rendering)

    t_vals = jnp.full_like(rays.near, fill_value=0.)
    weights = jnp.full_like(rays.near, fill_value=0.)
    renderings = []
    for i_level in range(self.num_levels):
      ...
      if compute_reg:
        (_, (t_vals, weights, rendering)), grad = jax.vmap(jax.value_and_grad(apply_fn, argnums=4, has_aux=True), in_axes=(None, None, 0, 0, 0, 0, 0, 0, 0, 0))(key, i_level, t_vals, weights, rays.origins, rays.directions, rays.radii, rays.near, rays.far, rays.viewdirs)
        rendering['reg'] = grad - (grad * rays.directions).sum(axis=-1, keepdims=True) * rays.directions
      else:
        (_, (t_vals, weights, rendering)) = jax.vmap(apply_fn, in_axes=(None, None, 0, 0, 0, 0, 0, 0, 0, 0))(key, i_level, t_vals, weights, rays.origins, rays.directions, rays.radii, rays.near, rays.far, rays.viewdirs)
...
def construct_mipnerf(rng, rays, config):
  ...
  init_variables = model.init(
      rng, rng=None, rays=ray, resample_padding=0., compute_extras=True, compute_reg=True)
...
@jax.jit
def softplus(x):
    return 1./100. * nn.softplus(100.*x)
...
class MLP(nn.Module):
  ...
  net_activation: Callable[Ellipsis, Any] = softplus  # The activation function.
