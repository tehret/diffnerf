...
class Config:
  ...
  batch_size: int = 1000  # The number of rays/pixels in each batch.
  batch_size_random: int = 1000  # The number of rays/pixels in each batch.
  ...
  # New loss function weights
  curvs_max: float = 20.0  # Clip curvatures larger than this.
  curvs_weight: float = 3e-4  # Loss weight for the curvature loss.
  reg_weight: float = 1e-7 # Loss weight for the new loss
  reg_max: float = 20. # Loss weight for the new loss
  ...
  train_render_every: int = 1000000  # Steps between test set renders for training
  n_input_views: int = 9  # Restrict the number of input views.
  #n_random_poses: int = 10000  # How many random poses to use.
  n_random_poses: int = 5000  # How many random poses to use.
